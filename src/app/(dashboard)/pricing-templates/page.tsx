    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {unpricedCount > 0 && (
            <p className="text-sm text-[#0E6E7E] mt-0.5">
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button type="submit" disabled={saving} className="flex-1 bg-[#0E6E7E] hover:bg-[#0A5560] text-white font-medium py-2 rounded-lg text-sm transition disabled:opacity-50">
          ...
          <div key={category} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {items.map(t => (
                <div key={t.id}>
                  {editingId === t.id ? (
                    <form onSubmit={handleUpdate} className="px-5 py-4 space-y-3 bg-[#EFF9FA]/50 border-l-2 border-[#0E6E7E]">
                      <p className="text-xs font-semibold text-[#0A5560] uppercase tracking-wider">Editing — {t.name}</p>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            required
                            className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0E6E7E] text-gray-900"
                          />
                        </div>
                        <div>
                        ...
                        <div>
                          <label className="block text-xs font-medium text-gray-600 mb-1">
                            Price ($){editForm.unit_price === 0 && <span className="ml-1 text-[#0E6E7E] font-semibold">← set a price</span>}
                          </label>
                          <input
                            type="number"
                            ...
                            min={0} step={0.01}
                            className={`w-full px-3 py-1.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0E6E7E] text-gray-900 ${
                              editForm.unit_price === 0 ? 'border-[#0E6E7E] bg-[#EFF9FA]' : 'border-gray-300'
                            }`}
                          />
                        </div>
                      </div>
                      <div className="flex gap-2">
                        ...
                        <button
                          type="submit"
                          disabled={editSaving}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0E6E7E] hover:bg-[#0A5560] text-white rounded-lg text-xs font-medium transition disabled:opacity-50"
                        >
                    ...
                    <div className={`flex items-center justify-between px-5 py-3.5 ${t.unit_price === 0 ? 'bg-[#EFF9FA]/40' : ''}`}>
                      <div>
                      ...
                      <div className="flex items-center gap-3">
                        {t.unit_price === 0 ? (
                          <span className="text-xs font-semibold text-[#0E6E7E] bg-[#D0F0F5] px-2 py-0.5 rounded-full">Price TBD</span>
                        ) : (
